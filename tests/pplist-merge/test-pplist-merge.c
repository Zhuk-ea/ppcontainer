...
int main(void) {
  ...
  // Тесты для ppList_merge
  printf("MERGE\n\n");

  // Тест 1: оба списка пусты
  ppList_CLEAR(l1); ppList_CLEAR(l2); ppList_CLEAR(anw1); ppList_CLEAR(anw2);
  printf("Test 1: Both lists empty\n");
  print_list((ppList*)&l1, "l1");
  print_list((ppList*)&l2, "l2");
  printf("\n");
  ppList_MERGE(l1, l2, cmp_int);
  check_list((ppList*)&l1, (ppList*)&empty, "l1", &errors);
  check_list((ppList*)&l2, (ppList*)&empty, "l2", &errors);
  printf("\n");
