// test-pp-list-splice-after.c - тестирование функции splice_after для итераторов по списку
#include <stdio.h>
#include "pplist.h"
...
int main(void) {
  ...
  //Тест 1: вставка src после головы
  ...
  printf("Test 1: Splice after head\n");
  ...
  // Тест 2: вставка src после элемента в середине
  ppList_CLEAR(l1); ppList_CLEAR(l2); ppList_CLEAR(anw); ppList_CLEAR(anw2);
  ...
  ppListIterator_GET_VAL(pos_val, pos);
  ppListIterator_GET_VAL(begin_val, src_begin);
  printf("Test 2: Splice after element in midle\n");
  printf("Dest position: after %d\n", pos_val);
  printf("Src range: [%d, NULL)\n", begin_val);
  ...
  // Тест 3: вставка src после хвоста (добавление в конец)
  ppList_CLEAR(l1); ppList_CLEAR(l2); ppList_CLEAR(anw); ppList_CLEAR(anw2);
  ...
  printf("Test 3: Splice after tail\n");
  printf("Dest position: after %d\n", pos_val);
  printf("Src range: [%d, NULL)\n", begin_val);
  ...
  // Тест 4: вставка после NULL (в начало списка)
  ppList_CLEAR(l1); ppList_CLEAR(l2); ppList_CLEAR(anw); ppList_CLEAR(anw2);
  ...
  ppListIterator_GET_VAL(begin_val, src_begin);
  printf("Test 4: Splice after NULL (insert before head)\n");
  ...
  // Тест 5: вставка диапазона из одного элемента после головы
  ppList_CLEAR(l1); ppList_CLEAR(l2); ppList_CLEAR(anw); ppList_CLEAR(anw2);
  ...
  printf("Test 5: Splice single element after head\n");
  printf("Dest position: after %d\n", pos_val);
  ...
  // Тест 6: перемещение диапазона (элементы 2 и 4) после элемента 7
  ppList_CLEAR(l1); ppList_CLEAR(l2); ppList_CLEAR(anw); ppList_CLEAR(anw2);
