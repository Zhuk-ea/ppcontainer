...
//------------------------------------------------------------------------------
int main(void) {
  // ? Эти настройки по сути могут быть заменены на вызов макроса(Что было сделано для l_int), или тут получается мешает то, что он внешне был создан, и там макрос бы не сработал? 
  ...
  printf("l_int after 4 push_back():\n");
  ...
  printf("l_int after 10 push_back():\n");
  ppList_print(stdout, (ppList*)&l_int);
